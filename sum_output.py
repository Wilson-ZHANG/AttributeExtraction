...
filename1 = 'disease_in_sum.csv'
save1 = open(filename1,'a+',encoding='utf-8')
filename2 = 'medicare_in_sum.csv'
...
for d in disease_list:
    ...
    for serious in serious_list:
        if(d[0] == serious[12]):
            print(serious[12])
            print(serious[0]+','+serious[1]+','+serious[2]+','+serious[3]+','+serious[4]+','+serious[5]+','+serious[6]+','+serious[7]+','+serious[8]+','+serious[9]+','+serious[10]+','+serious[11]+','+serious[12],file=save1)
    for others in others_list:
        if(d[0] == others[12]):
            print(others[12])
            print(others[0]+','+others[1]+','+others[2]+','+others[3]+','+others[4]+','+others[5]+','+others[6]+','+others[7]+','+others[8]+','+others[9]+','+others[10]+','+others[11]+','+others[12],file=save1)

for m in medicare_list:
    for cost in cost_list:
        if(m[0] == cost[12]):
            print(cost[12])
            print(cost[0]+','+cost[1]+','+cost[2]+','+cost[3]+','+cost[4]+','+cost[5]+','+cost[6]+','+cost[7]+','+cost[8]+','+cost[9]+','+cost[10]+','+cost[11]+','+cost[12],file=save2)
    for quote in quote_list:
        if(m[0] == quote[12]):
            print(quote[12])
            print(quote[0]+','+quote[1]+','+quote[2]+','+quote[3]+','+quote[4]+','+quote[5]+','+quote[6]+','+quote[7]+','+quote[8]+','+quote[9]+','+quote[10]+','+quote[11]+','+quote[12],file=save2)
