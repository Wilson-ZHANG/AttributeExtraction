...
def attr_get(filename):
    ...
    while(i<data_set.__len__()):
        ...
        if(data_set[i] == "保单备案时间"):
            try:
                time_pattern = "\[20[\d]+\]"
                time_match = re.search(time_pattern,s,re.M)
                time_begin = time_match.start()
                time_end = time_match.end()
                print(s[time_begin+1:time_end-1],file=save)
            except:
                print("保单备案时间提取失败。")
        try:
